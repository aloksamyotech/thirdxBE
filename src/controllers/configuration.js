...
export const addConfiguration = async (req, res) => {
  ...
  const addConfiguration = await configurationService.addConfiguration(configData)
...
export const updatedConfiguration = async (req, res) => {
  ...
  const updateDataConfiguration = await configurationService.updateConfiguration(configId, configData)
...
export const updateConfigurationStatus = async (req, res) => {
  ...
  const updateConfigurationStatus = await configurationService.updateConfigurationStatus(configId, isActive)
  res.status(statusCodes?.ok).send(updateConfigurationStatus)
}

export const deleteConfiguration = async (req, res) => {
  const { configId } = req?.params || {}
  const deleteConfiguration = await configurationService.deleteConfiguration(configId)
  res.status(statusCodes?.ok).send(deleteConfiguration)
}

export const searchConfigurationName = async (req, res) => {
  const { name } = req?.query || {}
  const searchConfigurationName = await configurationService.searchConfigurationByName(name)
